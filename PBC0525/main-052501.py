...
# define and load game background
start_bg_image = pygame.image.load("test/start_background/background.png").convert_alpha()
select_bg_image = pygame.image.load("test/select.png").convert_alpha()
...
# function for drawing select background
def draw_select_bg():
    scaled_bg = pygame.transform.scale(select_bg_image, (SCREEN_WIDTH, SCREEN_HEIGHT))
    screen.blit(scaled_bg, (0, 0))
...
while running:
    ...
    if current_animation_frames is not None:
        animation_speed = 0.2  # Adjust the animation speed as needed
